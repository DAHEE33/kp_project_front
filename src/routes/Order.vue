...
export default {
  data() {
    return {
      orderItems: [],
      userInfo: { username: "", phone: "", email: "" }, // 사용자 정보 (수정 가능)
      originalUserInfo: { userId: null }, // 변하지 않는 userId 저장
      isEditing: false,
      totalPrice: 0,
      discountAmount: 0,
    ...
    async fetchOrderPreview() {
      ...
      try {
        ...
        if (!this.originalUserInfo.userId) {
          this.originalUserInfo.userId = response.data.userId;
        }

        // ✅ 기존 localStorage 정보 유지하면서 백엔드에서 받은 값만 업데이트
        this.userInfo = {
          username: this.userInfo.username || response.data.username || "알 수 없음",
          phone: this.userInfo.phone || response.data.phoneNumber || "",
          email: this.userInfo.email || response.data.email || "",
        };

        // ✅ 수정된 사용자 정보를 localStorage에 저장
  ...
  created() {
    this.getUserInfoFromStorage(); // ✅ 새로고침 후 사용자 정보 유지
